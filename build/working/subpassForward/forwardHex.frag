...
layout(binding = 8) uniform sampler2D hexTex;
...
void main() {
	...
	float hex = 1.0 - texture(hexTex, uvNor * 5.0).r;
	float hexInterval = sin(ubo.times.x * 2.0) * 0.5 + 0.5;
	color.rgb += hexInterval * hex * pow(1.0 - dot(viewdir, normal), 3.0) * vec3(1.0, 0.5, 0.0);
	//color.rgb += hexInterval * hex * vec3(1.0, 0.5, 0.0);
