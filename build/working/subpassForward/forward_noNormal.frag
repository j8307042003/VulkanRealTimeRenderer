...
void main() {
	vec2 uvNor = uv;
	uvNor.y = 1 - uvNor.y;
	vec3 diffuse = texture(mainTex, uvNor).rgb;
	vec4 color = vec4(0);
	...
	vec3 lit = basicLit(worldNormal, -ubo.directionalLightDir.xyz, ubo.directionalLightColor.xyz, viewdir, diffuse.rgb);
