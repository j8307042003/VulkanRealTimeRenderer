...
VkSampler vkSys::Sampler::GetSampler(uint32_t samplerKind, int mipmaps)
{
	switch (samplerKind) 
	{
		case Sampler2D: return Create2DSampler(mipmaps);
		case Sampler2D_ClampEdge: return Create2DClampEdgeSampler(mipmaps);
	}
...
VkSampler vkSys::Sampler::Create2DClampEdgeSampler(int mipmaps)
{
	auto & vulkanInstance = *VkInstance::GetInstance();
