...
{
	vertexShader = createShaderModule(device, vertexName);
	fragmentShader = createShaderModule(device, fragmentName);
}	
...
VkPipeline Shader::BuildPipeline(const VkRenderPass & renderpass, const int & subpass, const VkPipelineLayout & pipelineLayout, const VkPipelineVertexInputStateCreateInfo * pCustomVertexInputState) const
{
	...
	if (vertexShader != VK_NULL_HANDLE) shaderStages.push_back(makeShaderStageCreateInfo(VK_SHADER_STAGE_VERTEX_BIT, vertexShader));
	if (fragmentShader != VK_NULL_HANDLE) shaderStages.push_back(makeShaderStageCreateInfo(VK_SHADER_STAGE_FRAGMENT_BIT, fragmentShader));
